...
/*static int mem_reallocAddr(void* bufferAddr, size_t newSize)
{
	void* origBuffer = *((void**)bufferAddr); // bufferAddr is really a void** ptr, but the prototype uses void* so any pointer can be passed without warnings
	void* newBuffer = mem_realloc(origBuffer, newSize);
	if(newBuffer != NULL)
	{
		*((void**)bufferAddr) = newBuffer;
		return 1;
	}
	else
	{
